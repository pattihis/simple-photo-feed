window.onload = function () {
    console.log('Simple Photo Feed Public JS Loaded');
    (function($) {
        ...
        class SpfJS {
            ...
            renderLightbox() {
                ...
                $(closeButton).attr('id', 'spf_lightbox_close')
                    .attr('href', '#')
                    .attr('title', 'Close lightbox')
                    .html('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><rect width="256" height="256" fill="none"/><line x1="160" y1="96" x2="96" y2="160" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16"/><line x1="96" y1="96" x2="160" y2="160" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16"/><circle cx="128" cy="128" r="96" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="16"/></svg>');

                let caption = document.createElement('div');
                $(caption).attr('id', 'spf_lightbox_caption');

                let link = document.createElement('a');
                $(link).attr('href', '#').attr('id', 'spf_lightbox_link')
                    .html('View on Instagram')
                    .attr('target', '_blank');

                let container = document.createElement('div');
                $(container).attr('id', 'spf_lightbox_container')
                    .append(imageBox)
                    .append(nextButton)
                    .append(prevButton)
                    .append(closeButton)
